...
abstract class LHSCompiler {
  ...
  final boolean isAtomLoaded(Atomic atom) {
    return atomPaths.containsKey(atom);
  }

  final boolean isMemLoaded(Membrane mem) {
    return memPaths.containsKey(mem);
  }
  ...
  List<Integer> getMemActuals() {
    ...
    for (int i = 0; i < mems.size(); i++) {
      if (memPaths.get(mems.get(i)) != null) args.add(
        memPaths.get(mems.get(i))
      );
    }
    return args;
  }
