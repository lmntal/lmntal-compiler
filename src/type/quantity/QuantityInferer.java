...
public class QuantityInferer {
  ...
  private void inferRule(RuleStructure rule) {
    ...
    // 右辺子膜の走査
    for (Membrane rhsmem : rule.rightMem.mems) inferRHSMembrane(rhsmem);
    // 右辺出現ルールの検査
    for (RuleStructure rhsrule : rule.rightMem.rules) inferRule(rhsrule);
  }
  ...
  private void inferRHSMembrane(Membrane rhs) {
    ...
    // 子膜の検査
    for (Membrane child : rhs.mems) inferRHSMembrane(child);
    // ルールの検査
    for (RuleStructure rule : rhs.rules) inferRule(rule);
  }
