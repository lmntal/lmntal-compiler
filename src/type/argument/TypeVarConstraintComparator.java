 ...
 *
 */
public class TypeVarConstraintComparator
  implements Comparator<TypeVarConstraint> {
  ...
  private int comparePath(
    Path p1,
    Path p2,
    TypeVarConstraint tvc1,
    TypeVarConstraint tvc2
  ) {
      ...
      if (p2 instanceof ActiveAtomPath) {
        ...
        if (cs == 0) {
          int cf = ap1
            .getFunctor()
            .getName()
            .compareTo(ap2.getFunctor().getName());
          if (cf == 0) {
            int ca = ap1.getFunctor().getArity() - ap2.getFunctor().getArity();
            if (ca == 0) return (
              tvc1.hashCode() - tvc2.hashCode()
            ); else return ca;
          } else return cf;
        } else return cs;
      } else return -1;
    } else if (p1 instanceof TracingPath) {
      if (p2 instanceof ActiveAtomPath) return 1; else if (
        p2 instanceof TracingPath
      ) {
        ...
        if (cp == 0) {
          int cf = tp1
            .getFunctor()
            .getName()
            .compareTo(tp2.getFunctor().getName());
          if (cf == 0) {
