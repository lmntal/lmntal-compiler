...
public class Tableau {
	...
	public static HashSet<Node> expand(Node node, HashSet<Node> nodes) {
		...
		if(node.newer.isEmpty()) {
			...
			while(it.hasNext()) {
				Node r = it.next();
				if(r.old.equals(node.old) && r.next.equals(node.next)) {
			...
			Node newNode = new Node(counter++, newIncoming, new HashSet<Link>(), newNext, new HashSet<Link>());
			nodes.add(node);
			return expand(newNode, nodes);
		} else { // New(q) is not empty
			Iterator<Link> it = node.newer.iterator();
			Link eta = it.next();
			node.newer.remove(eta);
			...
			if(eta.getAtom().getFunctor().getArity() == 1) {
				...
				if(eta.getAtom().getFunctor().getName().equals("false")) // contradiction
					return(nodes);
				Iterator<Link> oldIte = node.old.iterator();
				while(oldIte.hasNext()) {
					if(oldIte.next().getAtom().getName().equals("not"))
						if(oldIte.next().getAtom().nthAtom(0).getFunctor().equals(eta.getAtom().getFunctor()))
							return(nodes); // constadiction
				}
				HashSet<Integer> newIncoming = new HashSet<Integer>();	  
				newIncoming.addAll(node.incoming);
				HashSet<Link> newOld = new HashSet<Link>();
				newOld.addAll(node.old);
				newOld.add(eta);
				HashSet<Link> newNew = new HashSet<Link>();
				newNew.addAll(node.newer);
				HashSet<Link> newNext = new HashSet<Link>();
				newNext.addAll(node.next);
				Node newNode = new Node(counter++, newIncoming, newOld, newNew, newNext);
				return expand(newNode, nodes);
			} if(eta.getAtom().getFunctor().getName().equals("not")) {
				// the negation of a proposition
				// not, not(not), not(not(not)),... をどう扱うか？？
				HashSet<Integer> newIncoming = new HashSet<Integer>();	  
				newIncoming.addAll(node.incoming);
				HashSet<Link> newOld = new HashSet<Link>();
				newOld.addAll(node.old);
			...
			if(eta.getAtom().getFunctor().getName().equals("until")) {
				...
				Node newNode2 = new Node(counter++, newIncoming2, newOld2, newNew2, newNext2);
	...
	public static void toAutomaton(Membrane mem, HashSet<Node> nodes) {
		...
		while(nodeIte.hasNext()) {
			...
			Iterator idNameAtomIte = mem.atomIteratorOfFunctor(new SymbolFunctor("id",2));
			while(idNameAtomIte.hasNext()) { // src stateが存在するかどうか
				srcIdNameAtom = (Atom)idNameAtomIte.next();
				if(((IntegerFunctor)srcIdNameAtom.nthAtom(1).getFunctor()).intValue() == tmpNodeId) {
					existsSrc = true;
					break;
				}
			}
...
//System.out.println("edge: " + incomingId + "->" + tmpNodeId);
				...
				Iterator dstIdNameAtomIte = mem.atomIteratorOfFunctor(new SymbolFunctor("id",2));
				while(dstIdNameAtomIte.hasNext()) { // dst state が存在するかどうか
					newDstIdNameAtom = (Atom)dstIdNameAtomIte.next();
					if(((IntegerFunctor)newDstIdNameAtom.nthAtom(1).getFunctor()).intValue() == incomingId) {
						existsDst = true;
// System.out.println("exists: " + incomingId + "->" + tmpNodeId);
						break;
					}
				}
				Membrane dstMem;
				if(!existsDst) {
					...
					Atom newDstIdAtom = mem.newAtom(new IntegerFunctor(incomingId));
					Atom out = mem.newAtom(Functor.OUTSIDE_PROXY);
