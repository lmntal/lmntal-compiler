...
/** LMNtal中間命令を定義するフィールドであることを表します。 */
...
public class Instruction implements Cloneable {
  ...
  /** 命令の引数を保持する。 命令の種類によって引数の型が決まっている。 */
  ...
  /**
   * deref [-dstatom, srcatom, srcpos, dstpos] <br>
   * <strong><font color="#ff0000">出力するガード命令</font></strong><br>
   * アトム$srcatomの第srcpos引数のリンク先が第dstpos引数に接続していることを確認したら、 リンク先のアトムへの参照を$dstatomに代入する。
   */
  ...
  /**
   * derefatom [-dstatom, srcatom, srcpos] <br>
   * 出力する失敗しない最適化用＋型付き拡張用ガード命令<br>
   * アトム$srcatomの第srcpos引数のリンク先のアトムへの参照を$dstatomに代入する。
   *
   * <p>引き続き$dstatomが、単項アトム（整数なども含む）や自由リンク管理アトムと マッチするかどうか検査する場合に使用することができる。
   */
  ...
  /**
   * findatom [-dstatom, srcmem, funcref] <br>
   * 反復するガード命令<br>
  ...
  /**
   * lockmem [-dstmem, freelinkatom, memname] <br>
   * ロック取得するガード命令<br>
   * 自由リンク出力管理アトム$freelinkatomが所属する膜に対して、 ノンブロッキングでのロックを取得を試みる。 そしてロック取得に成功したこの膜への参照を$dstmemに代入する。
   * 取得したロックは、後続の命令列がその膜に対して失敗したときに解放される。
   *
   * <p>ロック取得に成功すれば、この膜はまだ参照を（＝ロックを）取得していなかった膜である （この検査はRuby版ではneqmem命令で行っていた）。
   *
  ...
  /**
   * anymem [-dstmem, srcmem, memtype, memname] <br>
   * 反復するロック取得するガード命令<br>
   * 膜$srcmemの子膜のうち、$memtypeで表せるタイプのまだロックを取得していない膜に対して次々に、 ノンブロッキングでのロック取得を試みる。
   * そして、ロック取得に成功した$memtypeで表せるタイプの各子膜への参照を$dstmemに代入する。 取得したロックは、後続の命令列がその膜に対して失敗したときに解放される。
   *
   ...
   * lock [srcmem] <br>
   * ロック取得するガード命令<br>
   * 膜$srcmemに対して、ノンブロッキングでのロックを取得を試みる。 取得したロックは、後続の命令列が失敗したときに解放される。
   *
  ...
  /**
   * natomsindirect [srcmem, countfunc] <br>
   * ガード命令<br>
  ...
  /**
   * dereffunc [-dstfunc, srcatom, srcpos] <br>
   * 出力する失敗しない拡張ガード命令 アトム$srcatomの第srcpos引数のリンク先のアトムのファンクタを取得し、$dstfuncに代入する。
   *
  ...
  /**
   * removeatom [srcatom, srcmem, funcref] <br>
   * ボディ命令<br>
   * （膜$srcmemにあってファンクタ$funcを持つ）アトム$srcatomを現在の膜から取り出す。 実行アトムスタックは操作しない。
   */
  ...
  /**
   * newatom [-dstatom, srcmem, funcref] <br>
   * ボディ命令<br>
   * 膜$srcmemにファンクタfuncrefを持つ新しいアトム作成し、参照を$dstatomに代入する。 アトムはまだ実行アトムスタックには積まれない。
   */
   ...
   * 何もしない。
   *
   * <p>$srcatomがどの膜にも属さず、かつこの計算ノード内の実行アトムスタックに積まれていないことを表す。 アトムを他の計算ノードで積んでいる場合、輸出表の整合性は大丈夫か調べる。 →
   * 輸出表は作らないことにしたので大丈夫。
   */
  ...
  /**
   * allocatomindirect [-dstatom, func] <br>
   * 型付き拡張用最適化用命令<br>
  ...
  /**
   * copyatom [-dstatom, mem, srcatom] <br>
   * 型付き拡張用ボディ命令 アトム$srcatomと同じ名前のアトムを膜$memに生成し、$dstatomに代入して返す。 実行アトムスタックは操作しない。
   *
  ...
  /**
   * removemem [srcmem, parentmem] <br>
   * ボディ命令<br>
   * 膜$srcmemを親膜（$parentmem）から取り出す。 <strike>膜$srcmemはロック時に実行膜スタックから除去されているため、実行膜スタックは操作しない。</strike>
  ...
  /**
   * newmem [-dstmem, srcmem, memtype] <br>
   * ボディ命令<br>
   * （活性化された）膜$srcmemに新しい（ルート膜でない）$memtypeで表せるタイプの子膜を作成し、 $dstmemに代入し、活性化する。
  ...
  /**
   * newroot [-dstmem, srcmem, nodeatom, memtype] <br>
   * ボディ命令<br>
   * 膜$srcmemの子膜にアトム$nodeatomの名前で指定された計算ノードで実行される新しいロックされた
   * $memtypeで表せるタイプのルート膜を作成し、参照を$dstmemに代入し、（ロックしたまま）活性化する。 この場合の活性化は、仮の実行膜スタックに積むことを意味する。
   *
   * <p>ただし上記の仕様は計算ノード指定が空文字列でないときのみ。 空文字列の場合は、newmemと同じだがロックされた状態で作られる。
   *
  ...
  /**
   * enqueueallatoms [srcmem] <br>
   * （予約された）ボディ命令<br>
   * 何もしない。または、膜$srcmemにある全てのアクティブアトムをこの膜の実行アトムスタックに積む。
   *
   * <p>アトムがアクティブかどうかを判断するには、 ファンクタを動的検査する方法と、2つのグループのアトムがあるとして所属膜が管理する方法がある。
   */
  ...
  /**
   * addmem [dstmem, srcmem] <br>
   * ボディ命令<br>
   * ロックされた（親膜の無い）膜$srcmemを（活性化された）膜$dstmemに移動する。 子膜のロックは取得していないものとする。
   * 子膜はルート膜の直前の膜まで再帰的に移動される。ホスト間移動した膜は活性化される。
   *
   ...
   * enqueuemem [srcmem]
   *
   * <p>ロックされた膜$srcmemをロックしたまま活性化する。 この場合の活性化は、$srcmemがルート膜の場合、仮の実行膜スタックに積むことを意味し、
   * ルート膜でない場合、親膜と同じ実行膜スタックに積むことを意味する。 すでに実行膜スタックまたは仮の実行膜スタックに積まれている場合は、何もしない。
   */
  ...
  /**
   * getlink [-link, atom, pos] <br>
   * 出力する失敗しない拡張ガード命令、最適化用ボディ命令<br>
  ...
  /**
   * newlink [atom1, pos1, atom2, pos2, mem1] <br>
   * ボディ命令<br>
   * アトム$atom1（膜$mem1にある）の第pos1引数と、 アトム$atom2の第pos2引数の間に両方向リンクを張る。
   *
  ...
  /**
   * relink [atom1, pos1, atom2, pos2, mem] <br>
   * ボディ命令<br>
   * アトム$atom1（膜$memにある）の第pos1引数と、 アトム$atom2の第pos2引数のリンク先（膜$memにある）の引数を接続する。
   *
  ...
  /**
   * unify [atom1, pos1, atom2, pos2, mem] <br>
   * ボディ命令<br>
   * アトム$atom1の第pos1引数のリンク先<strike>（膜$memにある）</strike>の引数と、
   * アトム$atom2の第pos2引数のリンク先<strike>（膜$memにある）</strike>の引数を接続する。
   * <strike>ただし$atom1および$atom2のリンク先がどちらも所属膜を持たない場合も許されており、 この場合、何もしないで終わってもよいことになっている。これは
   * f(A,A),(f(X,Y):-X=Y) の書き換えなどで起こる。</strike> $atom1 と $atom2 の両方もしくは一方が所属膜を持たない場合もある。 これは
   * a(A),f(A,B),(a(X),f(Y,Z):-Y=Z,b(X)) の書き換えなどで起こる。
   *
  ...
  /**
   * inheritlink [atom1, pos1, link2, mem] <br>
   * 最適化用ボディ命令<br>
   * アトム$atom1（膜$memにある）の第pos1引数と、 リンク$link2のリンク先（膜$memにある）を接続する。
   *
  ...
  /**
   * unifylinks [link1, link2, mem] <br>
   * ボディ命令<br>
   * リンク$link1の指すアトム引数とリンク$link2の指すアトム引数との間に双方向のリンクを張る。
   * ただし$link1は膜$memのアトムを指しているか、または所属膜の無いアトムを指している。 後者の場合、何もしないで終わってもよいことになっている。
   *
  ...
  /**
   * removetoplevelproxies [srcmem] <br>
   * ボディ命令<br>
  ...
  /**
   * insertproxies [parentmem,childmem] <br>
   * ボディ命令<br>
  ...
  /**
   * removetemporaryproxies [srcmem] <br>
   * ボディ命令<br>
  ...
  /**
   * copycells [-dstmap, dstmem, srcmem] <br>
   * （予約された）ボディ命令<br>
   * 再帰的にロックされた膜$srcmemの内容のコピーを作成し,膜$dstmemに入れる. その際、リンク先がこの膜の(子膜を含めて)中に無いアトムの情報を コピーされるアトムオブジェクト
   * {@literal -->} コピーされたアトムオブジェクト (2005/01/13 従来のAtom.idからの参照を変更) というMapオブジェクトとして,dstmapに入れる.
   */
   ...
   * dropmem [srcmem] <br>
   * （予約された）ボディ命令<br>
   * 再帰的にロックされた膜$srcmemを破棄する。 この膜や子孫の膜をルート膜とするタスクは強制終了する。
   */
   ...
   *
   * <p>srclinkのリンク先のアトムのコピーを$srcmapより得て、 そのアトムをリンク先とする-dstlinkを作って返す。
   */
   ...
   *
   * <p>linklistリストの各変数番号にはリンクオブジェクトが格納されている。 それらのリンクオブジェクトのリンク先は$mem内のアトムである。
   * それらのリンクオブジェクトの全ての組み合わせに対し、buddyの関係にあるかどうかを検査し、 その場合には'='アトムを挿入する。 挿入したアトムを$dstsetに追加する。
   */
   ...
   *
   * <p>linklistリストの各変数番号にはリンクオブジェクトが格納されている。 それらのリンクオブジェクトの全ての組み合わせに対し、buddyの関係にあるかどうかを検査し、
   * その場合には'='アトムを挿入する。 ただし'='は所属膜を持たない． 挿入したアトムを$dstsetに追加する。 この命令は型付きプロセス文脈の複製に伴い発行される．
   */
   ...
   *
   * <p>$srcsetに含まれる'='アトムをコピーしたアトムを$srcmapから得て、 削除し、リンクをつなぎなおす。<strike>$memはコピー先の膜。</strike>
   * 膜引数は廃止(2006/09/21)
   */
  ...
  /**
   * react [ruleref, [memargs...], [atomargs...], [varargs...]] <br>
   * 失敗しないガード命令<br>
   * ルールrulerefに対するマッチングが成功したことを表す。 処理系はこのルールのボディを呼び出さなければならない。
   *
   * <p>spec [formals, locals]; resetvars [[memargs...], [atomargs...], [varargs...]]; branch [body]
   * と同じ。 ただしbodyはrulerefのボディ命令列で、先頭の命令はspec[formals,locals]
   *
   * <p>（未使用命令）
   */
  ...
  /**
   * jump [instructionlist, [memargs...], [atomargs...], [varargs...]] <br>
   * 制御命令<br>
   * 指定の引数列でラベル付き命令列instructionlistを呼び出す。
   *
   * <p>指定した命令列の実行に失敗すると、この命令が失敗する。 指定した命令列の実行に成功すると、ここで終了する。
   *
   * <p>spec [formals, locals]; resetvars [[memargs...], [atomargs...], [varargs...]]; branch [body]
   * と同じ。 ただしbodyはinstructionlistの命令列で、先頭の命令はspec[formals,locals]
   */
  ...
  /**
   * commit [ruleref] <br>
   * 無視される最適化用およびトレース用命令<br>
   * 現在の実引数ベクタでルールrulerefに対するマッチングが成功したことを表す。 処理系は、この命令に到達するまでに行った全ての分岐履歴を忘却してよい。
   */
  @LMNtalIL public static final int COMMIT = 201;

  // static {setArgType(COMMIT, new ArgType(false, ARG_OBJ));}
  /**
   * 変更版 commit [rulename, lineno] トレースとデバッガに必要な情報を保持する. トレース用にルール名を文字列で保持.
   * デバッガ用にソース上の行番号linenoを整数で保持. sakurai
   */
  static {
  ...
  /**
   * resetvars [[memargs...], [atomargs...], [varargs...]] <br>
   * 失敗しないガード命令および最適化用ボディ命令<br>
   * 変数ベクタの内容を再定義する。新しい変数番号は、膜、アトム、その他の順番で0から振り直される。 <b>注意</b>　memargs[0]は本膜が予約しているため変更してはならない。
   */
  ...
  /**
   * changevars [[memargs...], [atomargs...], [varargs...]] <br>
   * 失敗しないガード命令および最適化用ボディ命令<br>
   * 変数ベクタの内容および長さを再定義する。 新しい変数番号は、膜、アトム、その他のいずれも0から振り直される。 ただしnullが入っている要素は無視される。
   * 同じ番号に異なる種類のオブジェクトが振られないように注意すること。 <b>注意</b>　memargs[0]は本膜が予約しているため変更してはならない。
   *
   * <p>（未使用命令）
   */
   ...
   *
   * <p>トップレベル命令列で使用された場合、ルールの適用が終わり、 再利用された全ての膜のロックを解放し、生成した全ての膜を活性化したことを表す。
   *
   * <p><b>注意</b>　proceedなしで命令列の終端まで進んだ場合、 その命令列の実行は失敗したものとする仕様が採用された。
   */
   ...
   * 構造化命令<br>
   * 引数の命令列を実行することを表す。 引数実行中に失敗した場合、引数実行中に取得したロックを解放し、branchの次の命令に進む。
   ...
   * 構造化命令<br>
   * 引数の命令列を実行することを表す。 引数実行中に失敗した場合、引数実行中に取得したロックを解放し、loopの次の命令に進む。
   ...
   * run [[instructions...]] <br>
   * （予約された）構造化命令<br>
   * 引数の命令列を実行することを表す。引数列はロックを取得してはならない。 引数実行中に失敗した場合、runの次の命令に進む。 引数実行中にproceed命令を実行した場合、次の命令に進む。
   *
  ...
  /**
   * not [instructionlist] <br>
   * （予約された）構造化命令<br>
   * 引数の命令列を実行することを表す。引数列はロックを取得してはならない。 引数実行中に失敗した場合、notの次の命令に進む。 引数実行中にproceed命令を実行した場合、この命令が失敗する。
   *
   ...
   *
   * <p>（基底項プロセスを指す）リンク列$srclinklistを$dstmemに複製し、 $dstlistの第1要素はコピーされたリンク列を，
   * 第二要素にはコピー元のアトムからコピー先のアトムへのマップがそれぞれ格納される． attrs を属性としたハイパーリンクは，コピー元の基底項に局所的ならばコピーされ
   * （つまり新たなハイパーリンクが作成され），局所的でないならばコピーされず共有される． 属性をもたないハイパーリンクやattrs に指定されていない属性のハイパーリンクは コピーされない．
   */
   ...
   *
   * <p>$srcmemに属する（基底項プロセスを指す）リンク列$srclinklistを現在の膜から取り出す。 実行アトムスタックは操作しない。
   */
   ...
   *
   * <p>基底項プロセス$srclinklistがどの膜にも属さず、かつスタックに積まれていないことを表す。
   */
  ...
  /**
   * isground [-natomsfunc, linklist, avolist, attrs] <br>
   * （予約された）ロック取得する拡張ガード命令<br>
   * リンク列$linklistの指す先が基底項プロセスであることを確認する。 すなわち、リンク先から（戻らずに）到達可能なアトムが全てこの膜に存在していることを確認する。
   * ただし、$avolistに登録されたリンクに到達したら失敗する。 見つかった基底項プロセスを構成するこの膜のアトムの個数（をラップしたInteger）を$natomsに格納する。
   *
   * <p>基底項プロセスのハイパーリンクはたどらないが，attrsに指定した属性のハイパーリンクで すべての端点がこの基底項プロセスに属する場合，そのハイパーリンクはこの基底項プロセス
   * の局所ハイパーリンクといい，後続のcopyground命令で（共有でなく）新規作成の対象となる．
   *
   * <p>natomsとnmemsと統合した命令を作り、$natomsの総和を引数に渡すようにする。 子膜の個数の照合は、本膜がロックしていない子膜の個数が0個かどうか調べればよい。
   * しかし本膜がロックしたかどうかを調べるメカニズムが今は備わっていないため、保留。
   *
   * <p>groundには膜は含まれないことになったので、上記は不要
   */
  ...
  /**
   * getclass [-stringatom, atom] <br>
   * 出力するガード命令<br>
   * アトム$atomがObjectFunctorまたはそのサブクラスのファンクタを持つことを確認し、 格納されたオブジェクトのクラスの完全修飾名文字列を表すファンクタを持つアトムを生成し、
   * $stringatomに代入する。 ただし、Translator を利用した場合、同一ソースのInlineコードで定義されたクラスに関しては単純名を取得する。(2005/10/17
   * Mizuno )
   */
   ...
   *
   * <p>$srcをリスト$dstlistの最後に追加する ( 2006/09/13 kudo )
   */
   ...
   * newhlink [link]
   *
   * <p>新たなhyperlinkを生成し, link先に接続することを示す
   */
   ...
   * makehlink [ID, link]
   *
   * <p>過去に生成されたhyperlinkのうち, 識別子IDを持つhyperlinkを生成し, link先に接続することを示す
   ...
   * ishlink [link]
   *
   * <p>link先に接続する構造がhyperlinkであることをチェックすることを示す
   */
   ...
   *
   * <p>膜memにあるunify_atom{@code ><}に対してhyperlinkの併合操作を行なうことを示す
   */
   ...
   *
   * <p>アトム番号atom1(価数=lenght1)の第arg1引数の型付きプロセス文脈が、
   ...
   *
   * <p>srclistの接続先の、attrsを属性としたhlground型の構造をdstlistの先にコピーする。 attrsに指定されていない属性のハイパーリンクのコピーしない。
   */
   ...
   *
   * <p>mem内のsrclistの接続先の、attrsを属性としたhlground型の構造を削除する。 attrsに指定されていない属性のハイパーリンクは削除しない。
   */
   ...
   *
   * <p>srclistの接続先のattrsを属性としたhlground型の構造のメモリを解放する。 attrsに指定されていない属性のハイパーリンクのメモリの解放しない。
   */
   ...
   *
   * <p>srclistに含まれるリンク群に接続している構造が、attrsを属性としたhlground型であることを検査し、探索したアトムの個数を natomsに格納する。
   ...
   *
   * <p>新たな属性付きhyperlinkを生成し, link先に接続することを示す。
   */
   ...
   *
   * <p>アトム$atomの第pos引数に格納されたリンクオブジェクトへの参照を$linkに代入する。
   */
  ...
  /**
   * iadd [-dstintatom, intatom1, intatom2] <br>
   * 整数用の組み込み命令<br>
  ...
  /**
   * iaddfunc [-dstintfunc, intfunc1, intfunc2] <br>
   * 整数用の最適化用組み込み命令<br>
  ...
  /**
   * iltfunc [intfunc1, intfunc2] <br>
   * 整数用の最適化用組み込みガード命令<br>
   ...
   *
   * <p>atom1 が atom2 のサブクラスかどうかを判定する 2006.6.30 by inui
   */
   ...
   *
   * <p>アトム $atom1 の第 $pos1 引数の接続先とアトム $atom2 の第 $pos2 引数の接続先を交換する
   */
  ...
  /** cyclelinks [alist, plist] アトムリスト $alist と番号リスト $plist によって表されるアトム引数列について、その接続先に対して巡回置換を行う */
   ...
   * clearlink [atom, pos]
   *
   * <p>アトム $atom の第 $pos 引数の接続先の情報をNULLにする命令。SLIM専用で、swaplink命令と併用して使われる。(2014/6/5 by aoyama)
   */
   ...
   * headatom [atom, mem]
   *
   * <p>SLIM専用の最適化命令。$mem 中のアトム $atom をそのファンクタのアトムリストのheadに持ってくる。--use-atomlistopのオプションが必須。(2014/6/8
   * by aoyama)
   */
   ...
   *
   * <p>SLIM専用の最適化命令。$mem 中のアトム $atom
   * の前にある部分アトムリストをアトムリストのtailにつなぎ替える。--use-atomlistopのオプションが必須。(2014/6/8 by aoyama)
   */
  ...
  /**
   * ダミー命令を生成する。 さしあたって生成メソッドがまだできてない命令はこれを使う
   *
   * @param s 説明用の文字列
   * @deprecated
   */
   ...
   *
   * @param list 命令列
   * @param varnum 変数番号 author n-kato
   */
  ...
  ////////////////////////////////////////////////////////////////
  /** この命令が出力命令の場合、出力の種類を返す。 そうでない場合、-1を返す。 */
  ...
  /**
   * この命令が副作用を持つ可能性があるかどうかを返す。不明な場合trueを返さなければならない。 ただし膜のロック取得は副作用とは見なさない。
   *
  ...
  /** この命令が制御動作をする可能性があるかどうかを返す。 制御動作とは失敗や反復などを表す。不明な場合trueを返さなければならない。 */
  ...
  /**
   * デバッグ用表示メソッド。 命令のkind (int)を与えると、該当する命令の名前 (String) を返してくれる。
   *
   * <p>author NAKAJIMA Motomu nakajima@ueda.info.waseda.ac.jp
   *
   * @return String
   */
   ...
   *
   * @return String
   *     <p>メモ：Instructionの中は、Listの中にArrayListが入れ子になって入っている。 つまり、【int型のname】命令 [引数1, 引数2, … , 引数n]
   *     ※一つのInstructionインスタンスには、1つしか命令がない ただし命令はInteger型、引数はObject。
   *     <p>メモ：出力の時、モードが−の変数をインデントする。
   */
