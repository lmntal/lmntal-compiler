...
public class Graph3DLayout implements Runnable{
	...
	private static final double DELTAEDGE = 0.5;//一回に移動するスケール(0.01?)	
	...
	protected void relax(runtime.AbstractMembrane m) {
		...
		for (Iterator i=m.atomIterator();i.hasNext();) {
			...
			for (int j=0;j<ne.length;j++) {
				...
				/*TODO: リンクの長さが十分ならば周りとの位置関係を気にする？*/
				else{

					for (Iterator ii=m.atomIterator();ii.hasNext();) {
						
						Node3D you = (Node3D)ii.next();
						if(me==you)continue;
						double mex = me.getPosition3d().x;
						double mey = me.getPosition3d().y;
						double mez = me.getPosition3d().z;
						double youx = you.getPosition3d().x;
						double youy = you.getPosition3d().y;
						double youz = you.getPosition3d().z;


						if(mex * youx <0)
							dx=abs(mex)+abs(youx);
						else dx=abs(mex - youx);
						
						if(mey * youy <0)
							dy=abs(mey)+abs(youy);
						else dy=abs(mey - youy);
						
						if(mez * youz <0)
							dz=abs(mez)+abs(youz);
						else dz=abs(mez - youz);
						
						dEdge=(dx*dx)+(dy*dy)+(dz*dz)-(EDGELENGTH*EDGELENGTH);
						if(dEdge > -DDEDGE)continue;
						if(dEdge>1.0)dEdge=1.0;
						if(dx < dy && dx < dz){
							if(mex < youx){
								mex -= DELTAEDGE * dEdge * dEdge;
