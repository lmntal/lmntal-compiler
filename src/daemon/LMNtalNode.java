...
//	}
	...
	public void close() {
		if(Env.debugDaemon > 0)System.out.println("LMNtalNode.close(): "+ socket);
		try {
			in.close();
			out.close();
...
//	}
	public static boolean isMyself(String ipstr) {
		...
		try {
			if(InetAddress.getLocalHost().getHostAddress().equals(ipstr)) result = true;  //TODO (nakajima)NAT対応
		} catch (java.net.UnknownHostException e) {
			e.printStackTrace();
		}
