...
my $isnert; # block index : int -> [ pos : int, type : int ]
my $titles; # block index : int -> title : string
my $dumps;  # block index : int -> dump : string
...
#print Dumper($dumps);
for($i=0;$i<=$#$titles;$i++) {
	$dumps->[$i] = my_split($dumps->[$i]);
}
#print Dumper($dumps);
for($i=0;$i<$#$titles;$i++) {
	my_diff($i, $dumps->[$i], $dumps->[$i+1]);
}
#print Dumper($insert);
...
$co = 0;
for($i=0;$i<$#$titles+1;$i++, $co++) {
	my $rule = $titles->[$i];
	my $show = display($rule);
	my $dump = $dumps->[$i];
	
	$dump = join '', @{ do_insert($i, $dump) };
...
sub my_diff {
	...
	my $d = diff($a, $b);
	#print Dumper($d);
	my $d0 = [ grep{ $#$_!=-1 } map{ [ map{ $_->[1] } grep{ $_->[0] eq "-" } @$_ ] } @$d ];
	my $d1 = [ grep{ $#$_!=-1 } map{ [ map{ $_->[1] } grep{ $_->[0] eq "+" } @$_ ] } @$d ];
	#print Dumper($d0);
	#print Dumper($d1);
	my $e0 = [ grep{elim($index, $_)} map{ [$_->[0], $_->[$#$_]] } @$d0 ];
	my $e1 = [ grep{elim($index+1, $_)} map{ [$_->[0], $_->[$#$_]] } @$d1 ];
	#print Dumper($e0);
	#print Dumper($e1);
	for $h(@$e0) {
		put($index+0, $h->[0], 0);
...
sub elim {
	my( $index, $a ) = @_;
	my $s = $dumps->[$index]->[$_->[0]];
	return $s !~ /^_/ and $s !~ /[(){},]/ if $_->[0]==$_->[1];
	return 1;
}

sub put {
	my( $index, $pos, $type )=@_;
	push @{$insert->[$index]}, [$pos, $type];
#	print "$index, $pos, $type\n";
}

# 順番が崩れないように最後から挿入していく
sub do_insert{
	my( $i, $dump ) = @_;
	#print Dumper($insert->[$i]);
	$insert->[$i] = [ sort { $b->[0] <=> $a->[0] } @{$insert->[$i]} ];
	#print Dumper($insert->[$i]);
	for my $ins(@{$insert->[$i]}) {
		splice(@$dump, $ins->[0]+0, 0, $tag->[$ins->[1]]);
	}
	$dump;
}
...
sub my_split {
	my( $a ) = @_;
	my @a = grep{$_ ne ""} split /(<[^>]+>|&nbsp;|_\d+|[a-zA-Z0-9]+)/, $a;
	#print Dumper([@a]);
	[@a];
}
